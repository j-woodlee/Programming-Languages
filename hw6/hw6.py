...
class Rename:
    ...
    def __init__(self, in_headers, args):
        self.input_headers = in_headers
        self.output_headers = list(self.input_headers)
        self.aggregate_headers = []
        self.oldHeader = args.pop(0)
        self.newHeader = args.pop(0)
        self.indexOfOldHeader = self.input_headers.index(self.oldHeader)

        if self.newHeader != self.oldHeader:
            self.output_headers[self.indexOfOldHeader] = self.newHeader
    ...
    def process_row(self,row):
        ...
        if self.newHeader == self.oldHeader:
            ...
            row[self.newHeader] = row.pop(self.oldHeader)
...
class Swap:
    ...
    def __init__(self, in_headers, args):
        ...
        self.output_headers = list(self.input_headers)
        self.aggregate_headers = []
        self.headerOne = args.pop(0)
        self.headerTwo = args.pop(0)
        # print(self.output_headers)
        indexOfHeader1 = self.input_headers.index(self.headerOne)
        indexOfHeader2 = self.input_headers.index(self.headerTwo)
        self.output_headers[indexOfHeader1] = self.headerTwo
        self.output_headers[indexOfHeader2] = self.headerOne
        # print(self.output_headers)
...
class Select:
    ...
    def __init__(self, in_headers, args):
        ...
        for arg in args:
            if arg in self.input_headers:
                self.output_headers.append(arg)

    def process_row(self, row):
        for e in self.input_headers:
            if e not in self.output_headers:
                del row[e]
...
class Filter:
    ...
    def process_row(self,row):
        return row if eval(self.expression, row) else None
...
class Update:
    ...
    def __init__(self, in_headers, args):
        ...
        self.output_headers = list(self.input_headers)
        self.aggregate_headers = []
        self.colName = args.pop(0)
        self.expression = args.pop(0)

    def process_row(self,row):
        if self.colName not in self.input_headers:
            raise Exception("Column name not in CSV file.")

        row[self.colName] = eval(self.expression, row)
        return row
...
class Add:
    ...
    def __init__(self, in_headers, args):
        ...
        if self.newCol not in self.input_headers:
            self.output_headers.append(self.newCol)
        else:
            raise Exception("Column already in database.")

    def process_row(self,row):
        row[self.newCol] = eval(self.expression, row)
        return row
...
class MaxBy:
    ...
    aggregate_headers should contain one column name, of the form:
      ...
      Are the display and value columns args[0] and args[1] respectively?
    """
    def __init__(self, in_headers, args):
        self.input_headers = in_headers
        self.output_headers = list(self.input_headers)
        self.displayCol = args[0]
        self.valueCol = args[1]
        self.aggregate_headers = ["Max " + self.displayCol + " By " + self.valueCol]
        self.aggregate_winner_value = 0
        self.aggregate_winner = ""

    def process_row(self,row):
        if float(row[self.valueCol]) > float(self.aggregate_winner_value): #how do I compare to the next row?
            self.aggregate_winner_value = row[self.valueCol]
            self.aggregate_winner = row[self.displayCol];
        return row

    def get_aggregate(self):
        return {"Max " + self.displayCol + " By " + self.valueCol : self.aggregate_winner}

        # return {self.aggregate_headers[0] : self.aggregate_winner}
...
class Sum:
    ...
    def __init__(self, in_headers, args):
        ...
        self.output_headers = list(self.input_headers)
        self.sum = 0
        self.summedColumn = args[0]
        self.aggregate_headers = [self.summedColumn + " Sum"]

    def process_row(self,row):
        self.sum += int(row[self.summedColumn])
        return row

    def get_aggregate(self): # aggregate row
        # print({(self.summedColumn + "Sum") : self.sum})
        return {(self.summedColumn + " Sum") : self.sum}
...
class Mean:
    ...
    def __init__(self, in_headers, args):
        self.input_headers = in_headers
        self.output_headers = list(in_headers)
        self.averagedColumn = args[0]
        self.aggregate_headers = [self.averagedColumn + " Mean"]
        self.sum = 0
        self.count = 0
    ...
    def process_row(self,row):
        # print(self.sum)
        # print(self.count)
        self.sum += int(row[self.averagedColumn])
        self.count += 1
        return row
    def get_aggregate(self):
        # print({self.averagedColumn + " Mean" : int(self.sum/self.count)})
        return {self.averagedColumn + " Mean" : round(self.sum/self.count)}
...
class ComposeQueries:
    ...
    def __init__(self, q1, q2):
        if q1.output_headers != q2.input_headers:
            raise Exception("query 1 output_headers do not match query 2 input_headers")
        self.q1 = q1
        self.q2 = q2
        self.input_headers = q1.input_headers
        self.output_headers = q2.output_headers
        if bool(set(q1.aggregate_headers) & set(q2.aggregate_headers)): # check that they the aggregate headers are distinct
            raise Exception("common elements found in aggregate_headers")

        self.aggregate_headers = q1.aggregate_headers + q2.aggregate_headers

    def process_row(self,row):
        # if self.q1.process_row(row) == None:
        #     return None
        self.q2.process_row(self.q1.process_row(row))
        return row

    def get_aggregate(self):
        return dict(self.q1.get_aggregate(), **self.q2.get_aggregate())
...
def buildQuery(in_headers, args):
    ...
    """
    split_arguments = splitArgs(args)
    constructedQueries = []
    next_input_headers = in_headers

    for arg in split_arguments:
        query = queries[arg[0][1:]] #get query
        constructedQuery = query(next_input_headers, arg[1:])
        next_input_headers = constructedQuery.output_headers
        constructedQueries.append(constructedQuery)

    finalQuery = ComposeQueries(constructedQueries[0],constructedQueries[1])
    constructedQueries = constructedQueries[2:]

    for a in constructedQueries:
        finalQuery = ComposeQueries(finalQuery, a)

    return finalQuery

def testBuildQueries():
    """
    Example: Show id of the player with the maximum steals per game.
    ...
    query = buildQuery(in_headers, ['-Add', 'stealsPerGame', 'int(stl)/int(gp)', '-MaxBy', 'id', 'stealsPerGame'])
...
def mostPointsPerMinute():
    ...
    """
    queryFile('player_career.csv', ['-Add', 'fullname', 'firstname + " " + lastname', '-Add', 'PointsPerMin', 'float(pts)/float(minutes) if float(minutes) > 0 else 0', '-MaxBy', 'fullname', 'PointsPerMin'])
...
def mostStealsPerGame():
    ...
    """
    queryFile('player_career.csv',['-Add', 'fullname', 'firstname + " " + lastname', '-Add', 'stealsPerGame', 'float(stl)/float(gp) if float(gp) > 0 else 0', '-MaxBy', 'fullname', 'stealsPerGame'])
...
def mosts():
    ...
    but no data row.
    """
    queryFile('player_career.csv', (['-Add', 'fullname', 'firstname + " " + lastname', '-MaxBy', 'fullname',
        'gp', '-MaxBy', 'fullname', 'minutes', '-MaxBy', 'fullname', 'pts', '-MaxBy', 'fullname', 'oreb', '-MaxBy',
        'fullname', 'dreb', '-MaxBy', 'fullname', 'reb', '-MaxBy', 'fullname', 'asts', '-MaxBy', 'fullname', 'stl', '-MaxBy',
        'fullname', 'blk', '-MaxBy', 'fullname', 'turnover', '-MaxBy', 'fullname', 'pf', '-MaxBy', 'fullname', 'fga', '-MaxBy',
        'fullname', 'fgm', '-MaxBy', 'fullname', 'fta', '-MaxBy', 'fullname', 'ftm', '-MaxBy', 'fullname', 'tpa', '-MaxBy', 'fullname', 'tpm']))
...
def bestPercentages():
    ...
    percentage (tpm/tpa).
    """
    queryFile('player_career.csv',['-Add', 'fullname', 'firstname + " " + lastname',
    '-Add', 'fieldGoalPercentage', 'float(fgm)/float(fga) if float(fga) > 0 else 0',
    '-Add', 'freeThrowPercentage', 'float(ftm)/float(fta) if float(fta) > 0 else 0',
    '-Add', 'threePointPercentage', 'float(tpm)/float(tpa) if float(tpa) > 0 else 0',
    '-MaxBy', 'fullname', 'fieldGoalPercentage',
    '-MaxBy', 'fullname', 'freeThrowPercentage',
    '-MaxBy', 'fullname', 'threePointPercentage'])
...
def count50Percents():
    ...
    queryFile('player_career.csv', (['-Add', 'fullname', 'firstname + " " + lastname', '-Add', 'fgp', 'float(fgm)/float(fga) if float(fga) > 0 else 0',
        '-Add', 'ftp', 'float(ftm)/float(fta) if float(fta) > 0 else 0', '-Add', 'tpp', 'float(tpm)/float(tpa) if float(tpa) > 0 else 0', '-Filter',
        'float(fgp) > .50', '-Filter', 'float(ftp) > .50', '-Filter', 'float(tpp) > .50', '-Count']))
